...
def StopMessage():
    return pb2.ClientBound(
        timestamp_us=int(time.monotonic() * 1000000), stop=pb2.Stop()
    )


def VideoMessage(data):
    return pb2.ClientBound(
        timestamp_us=int(time.monotonic() * 1000000), video=pb2.Video(data=data)
    )


def OverlayMessage(svg):
    return pb2.ClientBound(
        timestamp_us=int(time.monotonic() * 1000000), overlay=pb2.Overlay(svg=svg)
    )
...
class StreamingServer:
    ...
    def __init__(
        ...
        self._thread = threading.Thread(
            target=self._run, args=(mdns_name, tcp_port, web_port, annexb_port)
        )
        self._thread.start()
    ...
    def _run(self, mdns_name, tcp_port, web_port, annexb_port):
        try:
            with contextlib.ExitStack() as stack:
                logger.info(
                    "Listening on ports tcp: %d, web: %d, annexb: %d",
                    tcp_port,
                    web_port,
                    annexb_port,
                )
                ...
                while not self._done.is_set():
                    ...
                    for ready in rlist:
                        ...
                        if ready is tcp_socket:
                            client = ProtoClient(
                                name, sock, self._commands, self._camera.resolution
                            )
                        elif ready is web_socket:
                            client = WsProtoClient(
                                name, sock, self._commands, self._camera.resolution
                            )
    ...
    def write(self, data):
        ...
        if frame_type in ALLOWED_NALS:
            states = {
                client.send_video(frame_type, data) for client in self._enabled_clients
            }
    ...
    def _handle_stream_control(self, stream_control):
        ...
        with self._lock:
            if self._state == ClientState.DISABLED and not enabled:
                ...
                self._state in (ClientState.ENABLED_NEEDS_SPS, ClientState.ENABLED)
                and enabled
            ):
