...
_MODELS = {
    PLANTS: Model(
        labels=utils.load_labels("mobilenet_v2_192res_1.0_inat_plant_labels.txt"),
    ...
    INSECTS: Model(
        labels=utils.load_labels("mobilenet_v2_192res_1.0_inat_insect_labels.txt"),
    ...
    BIRDS: Model(
        labels=utils.load_labels("mobilenet_v2_192res_1.0_inat_bird_labels.txt"),
...
def sparse_configs(model_type, top_k=None, threshold=0.0):
    ...
    return {
        this_model.output_name: ThresholdingConfig(
            logical_shape=[num_labels],
            threshold=threshold,
