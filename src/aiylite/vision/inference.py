...
ModelDescriptor = namedtuple(
    "ModelDescriptor", ("name", "input_shape", "input_normalizer", "compute_graph")
)

ThresholdingConfig = namedtuple(
    "ThresholdingConfig", ("logical_shape", "threshold", "top_k", "to_ignore")
)

FromSparseTensorConfig = namedtuple(
    "FromSparseTensorConfig", ("logical_shape", "tensor_name", "squeeze_dims")
)
...
class ImageInference:
        ...
        try:
            ...
            if self._model_name not in self._engine.get_inference_state().loaded_models:
                ...
                self._stack.callback(
                    lambda: self._engine.unload_model(self._model_name)
                )
        except Exception:
            _close_stack_silently(self._stack)
            raise
    ...
    def run(self, image, params=None, sparse_configs=None):
        return self._engine.image_inference(
            self._model_name, image, params, sparse_configs
        )
    ...
    if isinstance(config, ThresholdingConfig):
        return pb2.SparseConfig(
            ...
            thresholding=pb2.SparseConfig.Thresholding(
                ...
                top_k=config.top_k,
                to_ignore=[
                    pb2.SparseConfig.Thresholding.ToIgnore(dim=d, label=l)
                    for d, l in config.to_ignore
                ],
            ),
        )
    ...
    if isinstance(config, FromSparseTensorConfig):
        return pb2.SparseConfig(
            ...
            from_sparse_tensor=pb2.SparseConfig.FromSparseTensor(
                tensor_name=config.tensor_name, squeeze_dims=config.squeeze_dims
            ),
        )
...
def _image_to_tensor(image):
    if isinstance(image, (bytes, bytearray)):
        ...
        return pb2.ByteTensor(
            shape=pb2.TensorShape(batch=1, height=0, width=0, depth=0), data=image
        )
...
_REQ_STOP_CAMERA_INFERENCE = _request_bytes(
    stop_camera_inference=pb2.Request.StopCameraInference()
)
_REQ_GET_INFERENCE_STATE = _request_bytes(
    get_inference_state=pb2.Request.GetInferenceState()
)
...
class InferenceEngine:
            ...
            self._communicate(
                pb2.Request(
                    load_model=pb2.Request.LoadModel(
                        ...
                        input_normalizer=pb2.TensorNormalizer(mean=mean, stddev=stddev),
    ...
    def unload_model(self, model_name):
        ...
        self._communicate(
            pb2.Request(unload_model=pb2.Request.UnloadModel(model_name=model_name))
        )
